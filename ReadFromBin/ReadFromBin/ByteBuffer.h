...
class ByteBuffer
{
    public:
		...
		static ByteBuffer CopyFromBytes(FILE* file)
		{
			ByteBuffer bf;

			uint8 content[1024] = {0};
			int count = fread(content, 1, 1024, file);
			...
			while(count)
			{
				...
				count = fread(content, 1, 1024, file);
			}
...
//            EndianConvert(value);
		...
		int32 ReadInt()
		{
			int leng = this->read<int32>();
			this->read_skip(sizeof(int32));
...
//                throw ByteBufferException();
		...
		template <typename T> T readWithOutSkip()
		{
			ResetBitPos();
			T r = read<T>(_rpos);
			return r;
